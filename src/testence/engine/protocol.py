...
class Engine(Protocol):
    ...
    def frame(self, target: Target) -> ContextManager[None]:
        """Scope locator actions, JavaScript evaluation, and predicate waits to an iframe."""
        ...
    ...
    def expect_visible(self, target: Target, timeout_ms: int | None = None) -> None:
        """Raise AssertionError for unmet visibility; other errors mean unavailable evidence."""
        ...

    # State assertions raise AssertionError when the state never holds; any other
    # error means the evidence was unavailable (see expect_visible).
    ...
    def eval_js(self, expression: str) -> Any:
        """Evaluate JavaScript in the active page or frame scope."""
        ...
    ...
    def wait_for_predicate_js(self, expression: str, timeout_ms: int | None = None) -> bool:
        ...
        matches the filter". It evaluates in the active page or frame scope. Prefer
        the specific waits above when one fits.
        """
        ...
