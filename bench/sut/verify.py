...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        results.append(
            case(
                "unfiltered list",
                "total 45, 10 rows, 0 skeletons",
                base,
                base["total"] == "45" and base["rows"] == 10 and base["skeletons"] == 0,
            )
        )
        type_search(page, "ember")
        filtered = observe(page)
        results.append(
            case("search applies on the first try", "total 2", filtered, filtered["total"] == "2")
        )
        ...
        first = observe(page)
        results.append(
            case("first search does nothing", "total still 45", first, first["total"] == "45")
        )
        ...
        second = observe(page)
        results.append(
            case("the identical repeat works", "total 2", second, second["total"] == "2")
        )
        ...
        results.append(
            case(
                "skeletons never give way",
                "10 skeletons, 0 real rows",
                stuck,
                stuck["skeletons"] == 10 and stuck["rows"] == 0,
            )
        )
        ...
        results.append(
            case("healthy: deep link filters", "total 2", with_filter, with_filter["total"] == "2")
        )
        open_page(page, "d34b", "search=ember&defects=D-34")
        ignored = observe(page)
        results.append(
            case("D-34: deep link ignored", "total 45", ignored, ignored["total"] == "45")
        )
        ...
        results.append(
            case(
                "list unchanged after the write",
                f"total still {before['total']}",
                after,
        ...
        results.append(
            case(
                "a reload shows it — which is what hides this defect",
                f"total {int(before['total']) + 1}",
                reloaded,
                reloaded["total"] == str(int(before["total"]) + 1),
            )
        )
        ...
        for run, flag, label, expected in [
            ...
            seen = observe(page)
            results.append(
                case(f"{flag}: {label}", f"total {expected}", seen, seen["total"] == expected)
            )
        open_page(page, "d43s", "defects=D-43-short-page")
        short = observe(page)
        results.append(
            case(
                "D-43-short-page: window short, total honest",
                "total 45 and 9 rows on a page of 10",
                short,
                short["total"] == "45" and short["rows"] == 9,
            )
        )
        ...
        results.append(
            case(
                "D-45: status filter dropped server-side",
                "total 45",
                unfiltered,
                unfiltered["total"] == "45",
            )
        ...
        inert = observe(page)
        results.append(
            case("typing alone is inert by design", "total 45", inert, inert["total"] == "45")
        )
        page.get_by_role("searchbox").press("Enter")
        page.wait_for_timeout(DEBOUNCE_SETTLE_MS)
        committed = observe(page)
        results.append(case("Enter commits", "total 2", committed, committed["total"] == "2"))

        print("\nC-08 controls: harmless changes that must stay green")
        for flag, expect in [
            ("U-01-renamed-control", "Add"),
            ("C-08-restyle", "Create"),
            ("C-08-reorder", "Create"),
            ("C-08-wrap", "Create"),
        ]:
            ...
            results.append(
                case(
                    f"{flag}: data unchanged",
                    f"total 45, submit says {expect!r}",
        ...
        results.append(
            case(
                "a write in one run does not move another's baseline",
        ...
        results.append(
            case(
                "an unknown id refuses to render an application",
                "an alert and no table",
                guarded,
