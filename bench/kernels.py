...
def _timed(fn, repeats: int = REPEATS) -> dict[str, float]:
    ...
    return {"ms_median": round(statistics.median(samples), 2),
    ...
    for r in range(runs):
        for seq in range(events_per_run):
            lines.append(json.dumps({
                "v": "testence/1", "run": f"r-{r}", "seq": seq,
                "ts": "2026-08-25T10:00:00.000Z", "kind": "step.end",
                "test": f"test_case_{seq % 20}", "step": f"s{seq}",
                "status": "ok", "duration_ms": 12.5 + seq % 40,
                "fingerprint": {"tag": "button", "role": None, "ariaLabel": "add",
                                "testid": None, "text": "add", "id": "add",
...
def _synth_fingerprints(count: int) -> list[dict]:
    return [{"tag": "button", "role": "button", "ariaLabel": f"label {i}",
             "testid": None, "text": f"caption {i}", "id": f"el-{i}",
             "classes": ["btn", f"variant-{i % 5}"]} for i in range(count)]


def main() -> None:
    results = {"backend": kernels.active_backend(), "python": sys.version.split()[0],
               "kernels": {}}
    ...
    results["kernels"]["parse_ledger"] = {
        **timing, "workload": f"{events} events / {len(ledger) // 1024} KiB",
    ...
    results["kernels"]["diff_aria"] = {
        **timing, "workload": "200 diffs of 500-node snapshots",
        "per_unit_us": round(timing["ms_median"] * 1000 / 200, 2)}

    # 3. Fingerprint scoring — rank 300 candidates, 100 drifted locators.
    target = {"tag": "button", "role": "button", "ariaLabel": "label 42",
              "testid": None, "text": "caption 42", "id": "el-42",
    ...
    results["kernels"]["score_candidates"] = {
        **timing, "workload": "100 rankings x 300 candidates",
    ...
    results["kernels"]["estimate_tokens"] = {
        **timing, "workload": f"{len(blob) // 1024 // 1024} MiB text"}
