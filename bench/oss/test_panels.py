...
def test_admin_checkbox(ex):
    ...
    ex.expect_visible(
        Target("css", "#exampleCheck1:checked"), intent="Selection is visibly checked"
    )
    ex.click(checkbox, intent="Disable the checkbox once")
    ex.expect_visible(Target("css", "#exampleCheck1:not(:checked)"), intent="Selection is cleared")
...
def test_admin_radio(ex):
    ...
    ex.expect_visible(Target("css", "#radio-2:checked"), intent="Second option is selected")
    ex.expect_visible(Target("css", "#radio-1:not(:checked)"), intent="First option is cleared")
...
def test_tabler_password(ex):
    ...
    ex.expect_visible(Target("css", "#signin-password[type=text]"), intent="Password is revealed")
    ex.click(Target("role", "button", name="Hide password"), intent="Hide password once")
    ex.expect_visible(Target("css", "#signin-password[type=password]"), intent="Password is masked")
...
def test_tabler_recovery(ex):
    ...
    ex.expect_visible(
        Target("role", "heading", name="Forgot password"), intent="Recovery form is visible"
    )
