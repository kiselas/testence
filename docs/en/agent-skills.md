...
manifest, and wheel distribution checks are implemented. The release receipt exercises
the installed `testence-triage` skill with real Codex and Claude Code clients against
one immutable evidence pack, then validates and submits both verdicts through Testence.
