...
| `test.end` | `nodeid`, `display_name`, canonical `status` (`passed`/`failed`/`broken`/`skipped`/`aborted`/`not_run`), `phase`, `duration_ms`, optional `pack`/error/xfail/xpass metadata, `error_kind` (`assertion`/`oracle`/`infrastructure`/`test_code`), `error_trace` (bounded, redacted), `screenshot` (run-relative, `evidence.screenshots: always`), `recordings` (`[{kind: trace\|video, path, redaction: none}]`, `evidence.trace`/`video`) |
