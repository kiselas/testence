# Документация Testence

## Как пользоваться Testence

- [Тестирование UI-функции](testing-a-feature.md) — полный рабочий процесс, начните отсюда
- [Конфигурация](configuration.md)
- [Аутентификация](auth.md)
- [Возможности engine и строгие действия](engine-capabilities.md)
- [Отчётность и экспортёры](reporting.md)
- [Схема доказательств](evidence-schema.md)
- [Multi-project quality packs](quality-packs.md)
- [Вычислительные ядра](kernels.md)
- [История изменений](CHANGELOG.md)
- [Политика поддержки](../../SUPPORT.md)

## Работа с агентами

- [Рабочий процесс агента](agent-workflow.md)
- [Переносимые Agent Skills](agent-skills.md)

## Устройство продукта

- [Архитектура](architecture.md)
- [Архитектурные решения](adr/README.md)
- [Дорожная карта](roadmap.md)

## Измерения и публичные заявления

- [Launch Benchmark Protocol](benchmark/launch-protocol.md) — correctness-first
  сравнение, frozen corpus и правила публичных заявлений
- [Корпус бенчмарка](benchmark/corpus.md)
- [Синтетическая таксономия отказов](benchmark/taxonomy.md)
- [Конкурентный бенчмарк](benchmark/competitive.md)
- [DemoSpec](demo-spec.md) — воспроизводимое demo «UI зелёный, данные не сохранились»
- [Сценарий 90-секундного demo video](demo-video-script.md)

## Инженерные записи

Это документы о том, как проект строится и выпускается, а не о том, как им
пользоваться. Они рабочие и не являются обещанием поддержки.

- [Launch Thesis](launch-thesis.md)
- [Позиционирование продукта](product-positioning.md)
- [Конкурентный ландшафт](competitive-landscape.md)
- [Аудит проекта](project-audit.md)
- [Готовность к open source](open-source-readiness.md)
- [Протокол пилота](pilot-protocol.md)
- [Аудиты](../audits/2026-09-13/README.md) и этапы релиза
  ([3](../stages/03-r1-release/README.md), [4](../stages/04-public-launch/README.md))

[English documentation](../en/README.md)
