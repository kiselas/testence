...
| Q5 | Что выносить в README после замера replay (Testence медленнее двух Playwright-участников) | Блок бенчмарков в README откладывается до `0.1.0a3`: сначала L18 (производительность), затем часть L05 о поимке ложно-зелёных тестов. `0.1.0a2` рекламирует интеграции, безопасность и workflow, а не скорость |
...
| L18 | P1 | Производительность прогона: отставание от Playwright-участников на свежем процессе | — | 4 | a2 | M |
| L19 | P1 | Базовый словарь DSL: проверки значения, URL, количества, состояния и атрибута; press, check, hover, select по подписи | — | 4 | a3 | M |
| L20 | P2 | Тесты жизненного цикла падают под `pytest -n` | — | 4 | a3 | S |

L05 стоит в `a3`, потому что цифры снимаются на финальном раннере. Замер replay (мини-часть,
фаза 3) выполнен и опубликован в методике бенчмарка; блок в README по решению Q5 появляется
только в `a3`, после L18 и замера корректности. L18 выполняется в фазе 4 первым.
...
Фаза 4  L18 → L01 (a2) → L19 → L12 → L11 раннер        ← сначала производительность
...
### L18 (P1). Производительность прогона

Добавлено по итогам мини-L05 (24.09.2026). **Проблема (замерено).** На свежем процессе
одного шестишагового теста Testence на 15% медленнее pytest-playwright и на 39% медленнее
Playwright Test (Windows 11, системный Chrome у всех, 30 раундов,
`bench/results/competitive-replay.json`). Базовый `91440a9` и текущий код дают одно
время, то есть фазы 1–2 не замедлили прогон. Жизненный цикл движка совпадает с чистым
Playwright (старт, навигация, остановка), значит, разница — около 0,5–1,1 с — в сессии
pytest и в шагах DSL.

**Решение.** Профилировать прогон и шаг DSL и убрать лишнее без потери evidence.
Кандидаты, которые нужно проверить замером: `fsync` на каждое событие ledger,
пересчёт digest всех ledger в run manifest, снятие fingerprint элемента после каждого
шага, ожидание «успокоения» страницы после действий, работу фикстур auth/api при
`auth: none`.

**Приёмка.** Медиана Testence на `bench/competitive/run.py --channel chrome` не хуже
pytest-playwright в пределах 95% ДИ; `bench/react_latency.py` и бюджет
`bench/scale_profile.py` в норме; содержимое ledger и pack не беднеет.

**Что показал замер (24.09.2026).** Метки на границах хуков pytest в обоих участниках
(Windows 11, Edge, один тест из шести шагов) разложили разницу около 270 мс так:

| Где | Testence − pytest-playwright | Причина |
|---|---|---|
| Импорт плагина | ≈ +145 мс | jsonschema (64 мс) грузится каждым прогоном через `contracts/document.py`; `urllib.request` и `http.client` — через фикстуру API |
| `sessionstart` | ≈ +80 мс | `platform.release()` — WMI-запрос на Windows с Python 3.12 (77 мс) |
| Контрольные точки manifest | ≈ 6 мс на точку | manifest перечитывает только что записанный ledger и прошлый manifest |
| Setup, тело теста, teardown | ≈ +20…40 мс | fingerprint элемента после шага (≈ 4,5 мс) и две записи ledger с fsync (≈ 2 мс) — это evidence, его не трогаем |

Исправлено без изменения записываемых значений: WMI-запрос идёт в потоке с момента
загрузки плагина (`testence/host.py`), jsonschema и API-клиент загружаются при первом
использовании, валидатор схемы кешируется, writer считает sha256 своего ledger по мере
записи. Импорт плагина сверх pytest и Playwright — 60 мс против 234 мс на `main`
(минимум из семи, машина под нагрузкой). Импорт Playwright при сборе тестов остаётся:
pytest-playwright платит его так же, а перенос проверки `capture_policy` в setup
ухудшил бы сообщение об ошибке настройки.

### L19 (P1). Базовый словарь DSL

Добавлено повторным аудитом (24.09.2026). **Проблема.** В DSL четыре проверки:
`expect_text`, `expect_visible`, `expect_hidden`, `expect_screenshot`. Нет проверки
значения поля, URL, числа элементов, enabled/disabled, checked и атрибута; нет `press`
(в движке есть), check/uncheck, hover и выбора option по подписи. Тестировщик,
пришедший с Playwright или Cypress, упирается в это в первый же день и уходит в
`ex.native`, где пропадают intent-шаги и evidence.

**Решение.** Каждый пункт — шаг DSL с intent, записью в ledger и fingerprint:

1. Проверки: `expect_value`, `expect_url(contains=… | equals=…)`, `expect_count`,
   `expect_enabled`, `expect_disabled`, `expect_checked(checked=True)`,
   `expect_attribute(name, value)`. Точное совпадение по умолчанию (как `expect_text`),
   провал — `AssertionError`, то есть `failed`, а не `broken`.
2. Действия: `press(key, target=None)` (фокус на цели и клавиша, протокол движка не
   меняется), `check`, `uncheck`, `hover`, `select(target, value=None, *, label=None)`.
3. Capabilities прежние: `browser.dom`, `browser.keyboard`. Сторонний движок без
   нового метода получает `UnsupportedCapability` с именем операции, а не
   `AttributeError`.
4. Навык `testence-author` и `testing-a-feature.md` (en/ru) перечисляют словарь.

**Приёмка.** На каждую проверку — позитивный тест, отрицательный (провал даёт
`AssertionError` с понятным текстом и статус `failed` в экспорте) и запись шага в
ledger; на каждое действие — тест на `tests/mock_app.py` или `data:`-странице.

### L20 (P2). Тесты жизненного цикла под xdist

Добавлено повторным аудитом. `tests/test_pytest_lifecycle.py` и
`tests/test_parallel_evidence.py` запускают вложенный pytest в том же процессе; под
`pytest -n 4` вложенный прогон наследует `PYTEST_XDIST_WORKER` и `TESTENCE_RUN_ID`
внешнего воркера и пишет не туда (5 падений на `main`). CI гоняет набор в один
процесс, поэтому пользователей это не задевает, но контрибьютор с `-n auto` видит
красное. Решение: фикстура, которая очищает эти переменные для вложенного прогона.
...
| L02 | влито | #22 | см. раздел 8.1 | trace/video с пометкой `redaction: "none"` — вместе с L12 |
| L07 | влито | #23 | см. раздел 8.2 | — |
| L03 | влито | #23 | см. раздел 8.2 | — |
| L08 | влито | #23 | см. раздел 8.2 | — |
| L09 | влито | #23 | см. раздел 8.2 | — |
| L06 | влито | #24 | `tests/test_native.py` | — |
| L14 | влито | #24 | — | — |
| L17 | влито | #24 | — | — |
...
### Версия 1.3 — повторный аудит после фаз 1–3

- Фазы 1–3 влиты (#22–#24). Флейк `rollback-during-window` на Windows-раннере
  исправлен виртуальными часами в тесте: корпус заморожен, менять его нельзя.
- L18 разобран по фазам хуков pytest; три причины из четырёх устранены (раздел L18).
  L18 идёт в `a2`: он безопасен и не меняет evidence.
- Добавлены L19 (базовый словарь DSL — проверки и действия, без которых
